...
type SeedTargets = Record<
  'users' | 'categories' | 'brands' | 'products' | 'orders' | 'orderItems' | 'reviews' | 'wishlists',
  number
>;

const FULL: SeedTargets = {
  users: 100_000,
  categories: 100,
  brands: 500,
  products: 1_000_000,
  orders: 5_000_000,
  orderItems: 20_000_000,
  reviews: 3_000_000,
  wishlists: 2_000_000,
};
const SMALL: SeedTargets = {
  users: 1_000,
  categories: 20,
  brands: 50,
  products: 10_000,
...
function targetScale(): SeedScale {
  return process.argv.includes('--scale=small') ? 'small' : 'full';
}

function randomDate(): Date {
  return faker.date.between({ from: '2021-01-01T00:00:00.000Z', to: new Date() });
}
...
function placeholders(rows: unknown[][]): { text: string; values: unknown[] } {
  ...
  const text = rows
    .map(
      (row, rowIndex) => `(${row.map((_, columnIndex) => `$${rowIndex * row.length + columnIndex + 1}`).join(', ')})`,
    )
    .join(', ');
  return { text, values };
}
...
async function insertBatches(
  ...
  for (let start = 1; start <= count; start += BATCH_SIZE) {
    ...
    if (end === count || end % (BATCH_SIZE * 20) === 0)
      console.log(`${table}: ${end.toLocaleString()}/${count.toLocaleString()}`);
  }
}
...
async function seed(): Promise<void> {
  ...
  const pool = new Pool({
    connectionString: process.env.DATABASE_URL ?? 'postgresql://postgres:postgres@localhost:5432/performance_lab',
  });
  console.log(
    `Seeding ${targetScale()} dataset (${Object.values(targets)
  ...
  try {
    ...
    await insertBatches(pool, 'users', ['name', 'email', 'country', 'city', 'created_at'], targets.users, (index) => {
      ...
      return [
        ...
        `user-${index}@performance-lab.test`,
        country,
        faker.location.city(),
        randomDate(),
      ];
    ...
    await insertBatches(
      pool,
      'products',
      ['name', 'description', 'brand_id', 'category_id', 'price', 'stock', 'rating', 'created_at'],
      targets.products,
      () => [
        faker.commerce.productName(),
        faker.commerce.productDescription(),
        faker.number.int({ min: 1, max: targets.brands }),
        faker.number.int({ min: 1, max: targets.categories }),
        money(2, 2_000),
        faker.number.int({ min: 0, max: 1_000 }),
        money(1, 5),
        randomDate(),
      ],
    );
    ...
    await insertBatches(pool, 'orders', ['user_id', 'status', 'total_amount', 'created_at'], targets.orders, () => [
      faker.number.int({ min: 1, max: targets.users }),
    ...
    await insertBatches(
      ...
      targets.orderItems,
      (index) => [
        Math.min(Math.ceil(index / 4), targets.orders),
        faker.number.int({ min: 1, max: targets.products }),
