...
export interface ProductFilters {
  ...
  createdTo?: string;
  search?: string;
  sortBy?: 'name' | 'price' | 'rating' | 'created_at' | 'stock';
  sortOrder?: 'asc' | 'desc';
}
...
export class ProductsRepository {
  ...
  buildListQuery(filters: ProductFilters, cursor?: number): SQL {
    ...
    if (filters.minPrice !== undefined && filters.maxPrice !== undefined)
      conditions.push(sql`p.price BETWEEN ${filters.minPrice} AND ${filters.maxPrice}`);
    ...
    if (filters.createdFrom && filters.createdTo)
      conditions.push(
        sql`p.created_at BETWEEN ${filters.createdFrom}::timestamptz AND ${filters.createdTo}::timestamptz`,
      );
    ...
    if (filters.search)
      conditions.push(sql`(p.name ILIKE ${`%${filters.search}%`} OR p.description LIKE ${`%${filters.search}%`})`);
