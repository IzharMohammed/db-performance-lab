...
const filtersFromQuery = (query: Record<string, string>): ProductFilters => ({
  ...
  category: numberOrUndefined(query.category),
  brand: numberOrUndefined(query.brand),
  minPrice: numberOrUndefined(query.minPrice),
  maxPrice: numberOrUndefined(query.maxPrice),
  minRating: numberOrUndefined(query.minRating),
  maxRating: numberOrUndefined(query.maxRating),
...
export class ProductsController {
  ...
  @Get() list(@Query() query: Record<string, string>): Promise<unknown> {
    return this.products.list(filtersFromQuery(query));
  }
  @Get('search') search(@Query() query: Record<string, string>): Promise<unknown> {
    return this.products.list(filtersFromQuery(query));
  }
  @Get('cursor') cursor(@Query() query: Record<string, string>): Promise<unknown> {
    return this.products.cursor(filtersFromQuery(query), numberOrUndefined(query.cursor));
  }
