...
export class UsersRepository {
  ...
  async list(country?: string, city?: string, page = 1, limit = 50): Promise<unknown> {
    ...
    return this.db
      .execute(sql`SELECT id, name, email, country, city, created_at FROM users WHERE 1 = 1 ${countryFilter} ${cityFilter}
  ...
  async orders(id: number): Promise<unknown> {
    const result = await this.db
      .execute(sql`SELECT u.id AS user_id, u.name, o.id AS order_id, o.status, o.total_amount, o.created_at
